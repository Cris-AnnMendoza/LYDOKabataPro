...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "update_status" && $appId) {
        ...
        $pdo->prepare("UPDATE scholarship_applications SET status=?,exam_score=?,qualification_score=?,rejection_reason=?,exam_scheduled=?,reviewed_by=?,reviewed_at=NOW() WHERE id=?")
    ...
    if ($action === "verify_doc") {
        ...
        $pdo->prepare("UPDATE scholarship_documents SET status=?,reviewed_by=?,reviewed_at=NOW() WHERE id=?")->execute([$status,$admin["id"],$docId]);
...
$statusColors = ["submitted"=>["#1565c0","#e3f2fd"],"under_review"=>["#f57f17","#fff8e1"],"for_exam"=>["#7b1fa2","#f3e5f5"],"passed"=>["#2e7d32","#e8f5e9"],"failed"=>["#c62828","#ffebee"],"approved"=>["#00796b","#e0f2f1"],"rejected"=>["#c62828","#ffebee"],"beneficiary"=>["#1565c0","#e3f2fd"]];
  ...
  <div style="display:flex;flex-direction:column;gap:16px">
    <div class="card">
      ...
      <div style="padding:4px 0">
        <?php
        $fields = ['Full Name'=>$viewApp['full_name'],'Email'=>$viewApp['email'],'Contact'=>$viewApp['contact_number'],'Age'=>$viewApp['age'],'Gender'=>$viewApp['gender'],'Address'=>$viewApp['address'],'School'=>$viewApp['school_name'],'Course'=>$viewApp['course'],'Year Level'=>$viewApp['year_level'],'GWA'=>$viewApp['gwa'],'Household Income'=>$viewApp['household_income']?''.number_format($viewApp['household_income'],2):'','Parent/Guardian'=>$viewApp['parent_name'],'Occupation'=>$viewApp['parent_occupation'],'Siblings'=>$viewApp['siblings']];
    ...
    <div class="card">
      ...
      <div style="padding:8px 18px">
        ...
        <div class="doc-row">
          ...
          <div style="flex:1"><div style="font-weight:600"><?=htmlspecialchars($docLabels[$doc['doc_type']]??$doc['doc_type'])?></div><div style="font-size:.75rem;color:#94a3b8"><?=htmlspecialchars($doc['original_name'])?></div></div>
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">
  <?php foreach (['submitted'=>'Submitted','under_review'=>'Under Review','for_exam'=>'For Exam','approved'=>'Approved'] as $s=>$l):
    ...
    <table class="tbl">
      ...
      ?>
        <tr>
          ...
          <td><?=$a['gwa']??''?></td>
          <td><?=$a['household_income']?''.number_format($a['household_income'],0):''?></td>
  ...
  <div class="modal-box" style="max-width:640px">
    ...
    <form method="POST" class="modal-body">
      ...
      <div class="modal-footer">
        <button type="button" class="btn-cancel" onclick="document.getElementById('addBatchModal').style.display='none'">Cancel</button>
        <button type="submit" class="btn-save"><i class="fas fa-save"></i> Create Batch</button>
      </div>
    </form>
  </div>
</div>

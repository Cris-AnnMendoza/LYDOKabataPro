...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    $pendingCount = 0;
    try {
        $pc = db()->query("SELECT (SELECT COUNT(*) FROM youth_users WHERE status='pending') + (SELECT COUNT(*) FROM admin_users WHERE status='pending') as total");
    ...
    <?php endif; ?>
    <?php navItem('programs.php','graduation-cap','Programs','manage_programs') ?>
    ...
    <?php navItem('#reports','chart-bar','Reports','view_reports') ?>
    <?php endif; ?>

    <?php if (hasPermission('manage_admins')): ?>
    <div class="nav-label">Administration</div>
    <?php navItem('admins.php','user-shield','Manage Admins','manage_admins') ?>
    <?php navItem('broadcast.php','broadcast-tower','Broadcast','manage_admins') ?>
    <?php navItem('logs.php','history','Activity Logs','manage_admins') ?>
    <?php navItem('blockchain.php','link','Submission Chain','manage_admins') ?>
    <?php endif; ?>
  </nav>

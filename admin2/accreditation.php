...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'verify_doc') {
        ...
        $pdo->prepare('UPDATE accreditation_documents SET status=?,reviewed_by=?,reviewed_at=NOW() WHERE id=?')
    ...
    if ($action === 'approve') {
        ...
        $pdo->prepare('UPDATE accreditation_applications SET status="approved",certificate_no=?,valid_until=?,reviewed_by=?,reviewed_at=NOW() WHERE id=?')
            ->execute([$certNo, $validUntil, $admin['id'], $appId]);
        // Complete all remaining workflow steps
        $pdo->prepare('UPDATE accreditation_workflow SET status="completed",done_at=NOW() WHERE application_id=? AND status="pending"')
            ->execute([$appId]);

        // Auto-create or link organization from accreditation
        $appRow = $pdo->prepare('SELECT * FROM accreditation_applications WHERE id=?');
        $appRow->execute([$appId]);
        $appData = $appRow->fetch();
        if ($appData) {
            // Check if org already exists with same name
            $existOrg = $pdo->prepare('SELECT id FROM organizations WHERE name=? LIMIT 1');
            $existOrg->execute([$appData['organization_name']]);
            $orgRow = $existOrg->fetch();
            if (!$orgRow) {
                $pdo->prepare('INSERT INTO organizations (name,category,barangay,org_email,is_active) VALUES (?,?,?,?,1)')
                    ->execute([$appData['organization_name'], $appData['category'] ?? '', $appData['barangay'] ?? '', $appData['contact_email'] ?? '']);
                $orgId = (int)$pdo->lastInsertId();
            } else {
                $orgId = (int)$orgRow['id'];
                $pdo->prepare('UPDATE organizations SET is_active=1 WHERE id=?')->execute([$orgId]);
            }
            // Link submitter as President/Leader with can_invite=1
            $pdo->prepare('INSERT INTO organization_members (organization_id,user_id,role,position,can_invite,is_active,joined_at)
                VALUES (?,?,"Leader","President",1,1,CURDATE())
                ON DUPLICATE KEY UPDATE role="Leader",position="President",can_invite=1,is_active=1')
                ->execute([$orgId, $appData['submitted_by']]);
        }

        // Notify applicant
        $pdo->prepare('INSERT INTO notifications (user_id,type,title,category,message,is_read) VALUES (?,?,?,?,?,0)')
            ->execute([$appData['submitted_by'], 'approval', 'Accreditation Approved', 'approval',
                "Your organization \"{$appData['organization_name']}\" has been accredited! Certificate No: $certNo"]);
    ...
    if ($action === 'reject') {
        ...
        $pdo->prepare('UPDATE accreditation_applications SET status="rejected",rejection_reason=?,reviewed_by=?,reviewed_at=NOW() WHERE id=?')
    ...
    if ($action === 'under_review') {
        ...
        $pdo->prepare('UPDATE accreditation_workflow SET status="completed",done_at=NOW() WHERE application_id=? AND step="Document Verification"')
            ->execute([$appId]);
        ...
        <tr>
          ...
          <td>
            <span style="font-size:.82rem">
              <span style="color:#2e7d32;font-weight:700"><?=$app['verified_count']?></span>
              <span style="color:#94a3b8">/<?=$app['doc_count']?> verified</span>
            </span>
          </td>

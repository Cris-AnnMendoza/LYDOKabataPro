...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "join_org") {
        ...
        if ($orgId) {
            ...
            $chk = $pdo->prepare('SELECT id FROM organization_members WHERE organization_id=? AND user_id=? AND is_active=1');
            $chk->execute([$orgId, $userId]);
            if ($chk->fetch()) {
                $error = 'You are already a member of this organization.';
            } else {
                $pdo->prepare('INSERT INTO org_join_requests (organization_id,user_id,message) VALUES (?,?,?)
                    ON DUPLICATE KEY UPDATE message=VALUES(message),status="pending",reviewed_at=NULL')
                    ->execute([$orgId, $userId, $msg]);
                $success = 'Join request submitted. Wait for approval from the organization or admin.';
            }
        }
    }
    ...
    if ($action === "leave_org") {
        $orgId = (int)($_POST['org_id'] ?? 0);
        if ($orgId) {
            $pdo->prepare('UPDATE organization_members SET is_active=0 WHERE organization_id=? AND user_id=?')
...
<div class="tabs">
  ...
  <a href="?tab=orgs"      class="tab-btn <?=$tab==='orgs'?'active':''?>"><i class="fas fa-sitemap"></i> My Organizations</a>
      ...
      <div class="form-row-2">
        <div class="fg"><label>Position / Role</label>
          <select name="organization_role">
            ...
            <?php foreach(['President','Vice President','Secretary','Treasurer','Auditor','PRO','Sergeant-at-Arms','Member','Officer','Coordinator','Adviser','Other'] as $pos): ?>
            <option value="<?=$pos?>" <?=($user['organization_role']??'')===$pos?'selected':''?>><?=$pos?></option>
            <?php endforeach; ?>
          </select>
        </div>
...
$myOrgs = $pdo->prepare(
    'SELECT o.*, om.role, om.position, om.can_invite, om.joined_at,
     ...
     GROUP BY o.id, om.role, om.position, om.can_invite, om.joined_at
     ORDER BY om.joined_at DESC'
);
$myOrgs->execute([$userId]);
$myOrgList = $myOrgs->fetchAll();

// Load pending join requests
$myRequests = $pdo->prepare(
    'SELECT r.*, o.name as org_name FROM org_join_requests r
     JOIN organizations o ON o.id=r.organization_id
     WHERE r.user_id=? ORDER BY r.created_at DESC'
);
$myRequests->execute([$userId]);
$myJoinReqs = $myRequests->fetchAll();

// All active orgs not already a member of
$allOrgs = $pdo->prepare(
    'SELECT o.id, o.name, o.category, o.barangay,
     (SELECT COUNT(*) FROM organization_members m WHERE m.organization_id=o.id AND m.is_active=1) as member_count
     FROM organizations o
     WHERE o.is_active=1
     AND o.id NOT IN (SELECT organization_id FROM organization_members WHERE user_id=? AND is_active=1)
     ORDER BY o.name'
);
$allOrgs->execute([$userId]);
$availableOrgs = $allOrgs->fetchAll();
?>
<div class="profile-card">
  <div class="profile-card-header"><i class="fas fa-sitemap"></i><h3>My Organizations</h3></div>
  <div class="profile-card-body">
    <?php if (empty($myOrgList)): ?>
      <p style="color:#94a3b8;font-size:.88rem;text-align:center;padding:20px 0">You are not a member of any organization yet.</p>
    <?php else: foreach ($myOrgList as $o): ?>
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 0;border-bottom:1px solid #f1f5f9;flex-wrap:wrap;gap:8px">
      <div>
        <div style="font-weight:700;font-size:.92rem"><?=htmlspecialchars($o['name'])?></div>
        <div style="font-size:.78rem;color:#475569;margin-top:2px;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          <span style="background:#e3f2fd;color:#1565c0;padding:2px 8px;border-radius:50px;font-size:.7rem;font-weight:700"><?=htmlspecialchars($o['position'] ?: $o['role'])?></span>
          <?php if ($o['barangay']): ?><span><?=htmlspecialchars($o['barangay'])?></span><?php endif; ?>
          <?php if ($o['joined_at']): ?><span>Joined <?=date('M Y', strtotime($o['joined_at']))?></span><?php endif; ?>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:8px">
        <?php $netColor = $o['net_pts'] >= 0 ? '#2e7d32' : '#c62828'; $netBg = $o['net_pts'] >= 0 ? '#e8f5e9' : '#ffebee'; ?>
        <div style="display:flex;gap:6px;align-items:center;font-size:.78rem">
          <span style="color:#2e7d32;font-weight:700">+<?=$o['merit_pts']?> merit</span>
          <span style="color:#94a3b8">/</span>
          <span style="color:#c62828;font-weight:700">-<?=$o['demerit_pts']?> demerit</span>
          <span style="background:<?=$netBg?>;color:<?=$netColor?>;padding:2px 8px;border-radius:50px;font-size:.72rem;font-weight:800"><?=($o['net_pts']>=0?'+':'').$o['net_pts']?> net</span>
        </div>
        <form method="POST" onsubmit="return confirm('Leave this organization?')">
          <input type="hidden" name="action" value="leave_org"/>
          <input type="hidden" name="org_id" value="<?=$o['id']?>"/>
          <button type="submit" style="padding:5px 12px;background:#ffebee;color:#c62828;border:1.5px solid #ef9a9a;border-radius:7px;font-family:inherit;font-size:.78rem;font-weight:700;cursor:pointer">
...
<?php if (!empty($myJoinReqs)): ?>
<div class="profile-card">
  <div class="profile-card-header"><i class="fas fa-clock"></i><h3>Pending Join Requests</h3></div>
  <div class="profile-card-body">
    <?php foreach ($myJoinReqs as $r): ?>
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f1f5f9">
      <div style="font-size:.88rem;font-weight:600"><?=htmlspecialchars($r['org_name'])?></div>
      <?php
        $sc = $r['status']==='pending' ? '#f57f17' : ($r['status']==='approved' ? '#2e7d32' : '#c62828');
        $bg = $r['status']==='pending' ? '#fff8e1' : ($r['status']==='approved' ? '#e8f5e9' : '#ffebee');
      ?>
      <span style="background:<?=$bg?>;color:<?=$sc?>;padding:3px 10px;border-radius:50px;font-size:.72rem;font-weight:700"><?=ucfirst($r['status'])?></span>
    </div>
    <?php endforeach; ?>
  </div>
</div>
...
<div class="profile-card">
  <div class="profile-card-header"><i class="fas fa-search"></i><h3>Find & Join an Organization</h3></div>
  <div class="profile-card-body">
    <?php if (empty($availableOrgs)): ?>
      <p style="color:#94a3b8;font-size:.88rem;text-align:center;padding:16px 0">No other organizations available to join.</p>
    <?php else: foreach ($availableOrgs as $o): ?>
    <div style="border:1.5px solid #e2e8f0;border-radius:10px;padding:14px 16px;margin-bottom:10px">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px">
        <div>
          <div style="font-weight:700;font-size:.92rem"><?=htmlspecialchars($o['name'])?></div>
          <div style="font-size:.78rem;color:#475569;margin-top:3px">
            <?php if ($o['category']): ?><span style="background:#f1f5f9;color:#475569;padding:2px 7px;border-radius:50px;font-size:.7rem;font-weight:600"><?=htmlspecialchars($o['category'])?></span><?php endif; ?>
            <?php if ($o['barangay']): ?> &middot; <?=htmlspecialchars($o['barangay'])?><?php endif; ?>
            &middot; <?=$o['member_count']?> member<?=$o['member_count']!=1?'s':''?>
          </div>
        </div>
        <button onclick="document.getElementById('joinForm<?=$o['id']?>').style.display=document.getElementById('joinForm<?=$o['id']?>').style.display==='none'?'block':'none'"
          style="padding:6px 14px;background:#e3f2fd;color:#1565c0;border:1.5px solid #90caf9;border-radius:8px;font-family:inherit;font-size:.8rem;font-weight:700;cursor:pointer;flex-shrink:0">
          <i class="fas fa-user-plus"></i> Request to Join
        </button>
      </div>
      <div id="joinForm<?=$o['id']?>" style="display:none;margin-top:12px">
        <form method="POST">
          <input type="hidden" name="action" value="join_org"/>
          <input type="hidden" name="org_id" value="<?=$o['id']?>"/>
          <textarea name="message" rows="2" placeholder="Optional: introduce yourself or explain why you want to join..."
            style="width:100%;padding:8px 12px;border:1.5px solid #e2e8f0;border-radius:8px;font-family:inherit;font-size:.85rem;outline:none;resize:vertical;margin-bottom:8px"></textarea>
          <button type="submit" style="padding:8px 18px;background:#1565c0;color:#fff;border:none;border-radius:8px;font-family:inherit;font-size:.85rem;font-weight:600;cursor:pointer">

...
$docTypeLabels = ['request_letter'=>'Letter Request (addressed to the Municipal Mayor through LYDO)','project_proposal'=>'Project Proposal / Activity Plan','participant_list'=>'List of Participants','sk_endorsement'=>'SK Endorsement (if applicable)','other'=>'Other Document'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'submit') {
        ...
        if (isset($_POST['check_only'])) {
            ...
            $orgIdTxt= trim($_POST['organization_id_text'] ?? '');
            ...
            if (!$orgId || !$title || !$desc || !$repName || !$contact) {
                ...
                if (!empty($checkResult['doc_issues'])) {
                    ...
                    $pdo->prepare('INSERT INTO assistance_requests
                        (organization_id,organization_id_text,submitted_by,representative_name,contact_number,contact_email,
                         title,activity_type,description,objectives,expected_outcome,participants,target_date,target_venue,budget_requested,status)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,"submitted")')
                        ->execute([$orgId,$orgIdTxt,$userId,$repName,$contact,$email,
                                   $title,$actType,$desc,$obj,$outcome,$parts,$tDate?:null,$venue,$budget]);
                    $reqId = (int)$pdo->lastInsertId();

                    // Record on hash chain
                    require_once __DIR__ . '/../shared/blockchain.php';
                    blockchain_record($pdo, 'assistance', $reqId, $userId, 'submitted', [
                        'title' => $title, 'org_id' => $orgId, 'activity_type' => $actType
                    ]);
                    ...
                    foreach ($sessionFiles as $type => $info) {
                        ...
                        require_once __DIR__ . '/../shared/blockchain.php';
                        $fileHash = blockchain_hash_file($pdo, $uploadDir . $fname, $type, 'assistance', $reqId, $userId);
                        $pdo->prepare('INSERT INTO assistance_documents (request_id,doc_type,filename,original_name,file_size,file_hash) VALUES (?,?,?,?,?,?)')
                            ->execute([$reqId, $type, $fname, $info['name'], $info['size'], $fileHash]);
                    }
                    ...
                    $admins = $pdo->query('SELECT id FROM admin_users WHERE is_active=1')->fetchAll(PDO::FETCH_COLUMN);
...
//  Load user orgs (only orgs the user is a member of) and requests 
$userOrgs = $pdo->prepare(
    'SELECT o.id, o.name, om.position, om.can_invite
     FROM organizations o
     JOIN organization_members om ON om.organization_id=o.id
     WHERE om.user_id=? AND om.is_active=1 AND o.is_active=1
     ORDER BY o.name'
);
$userOrgs->execute([$userId]);
$userOrgs = $userOrgs->fetchAll();
...
$docTypeLabels = ['request_letter'=>'Letter Request (to Municipal Mayor through LYDO)','project_proposal'=>'Project Proposal / Activity Plan','participant_list'=>'List of Participants','sk_endorsement'=>'SK Endorsement (if applicable)','other'=>'Other'];
?>
  ...
  <div style="display:flex;flex-direction:column;gap:16px">
    <div class="card">
      ...
      <div style="padding:4px 0">
        ...
        <div class="info-row" style="padding:8px 18px"><span class="info-label">Representative</span><span class="info-val"><?=htmlspecialchars($viewReq['representative_name']?:'')?></span></div>
        <div class="info-row" style="padding:8px 18px"><span class="info-label">Activity Type</span><span class="info-val"><?=htmlspecialchars($viewReq['activity_type']?:'')?></span></div>
        <div class="info-row" style="padding:8px 18px"><span class="info-label">Target Date</span><span class="info-val"><?=$viewReq['target_date']?date('F j, Y',strtotime($viewReq['target_date'])):''?></span></div>
        <div class="info-row" style="padding:8px 18px"><span class="info-label">Venue</span><span class="info-val"><?=htmlspecialchars($viewReq['target_venue']?:'')?></span></div>
        <div class="info-row" style="padding:8px 18px"><span class="info-label">Participants</span><span class="info-val"><?=number_format($viewReq['participants'])?></span></div>
        <div class="info-row" style="padding:8px 18px"><span class="info-label">Budget</span><span class="info-val">&#8369;<?=number_format($viewReq['budget_requested'],2)?></span></div>
  ...
  <div style="padding:14px 18px">
    ...
    <a href="?view=<?=$r['id']?>" class="req-card">
      <div>
        ...
        <div class="req-meta"><?=htmlspecialchars($r['org_name'])?> &middot; <?=$r['target_date']?date('M j, Y',strtotime($r['target_date'])):'No date set'?></div>
      </div>
...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" enctype="multipart/form-data">
      ...
      ?>
      <div class="doc-grid">
        <?php foreach (['request_letter'=>'Letter Request (to Municipal Mayor through LYDO)','project_proposal'=>'Project Proposal / Activity Plan','participant_list'=>'List of Participants','sk_endorsement'=>'SK Endorsement (if applicable)'] as $type => $label):

    ...
    if ($action === "apply") {
        ...
        if (!$b) { $error = "This batch is no longer accepting applications."; }
            ...
            if ($dup->fetch()) { $error = "You already applied for this batch."; }
                ...
                $pdo->prepare("INSERT INTO scholarship_applications (batch_id,user_id,full_name,birthdate,age,gender,address,contact_number,email,school_name,course,year_level,gwa,household_income,parent_name,parent_occupation,siblings) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
                    ->execute([$batchId,
                        $userId,
                ...
                foreach ($docLabels as $type => $label) {
                    ...
                    $pdo->prepare("INSERT INTO scholarship_documents (application_id,doc_type,filename,original_name,file_size) VALUES (?,?,?,?,?)")

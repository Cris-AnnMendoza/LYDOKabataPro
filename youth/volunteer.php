...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
    ...
    if ($action === "register") {
        ...
        $prog = $pdo->prepare("SELECT * FROM volunteer_programs WHERE id=? AND is_active=1");
        ...
        if (!$program) { $error = "Program not found."; }
            ...
            if ($age < $program["min_age"] || $age > $program["max_age"]) {
                ...
                if ($dup->fetch()) { $error = "You are already registered for this program."; }
                    ...
                    $pdo->prepare("INSERT INTO volunteer_registrations (user_id,program_id,full_name,age,birthdate,gender,address,contact_number,email,school_org,emergency_name,emergency_number) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
                        ->execute([$userId,$programId,trim($_POST["full_name"]),$age,$_POST["birthdate"],$_POST["gender"],trim($_POST["address"]),trim($_POST["contact_number"]),trim($_POST["email"]),trim($_POST["school_org"]??""),trim($_POST["emergency_name"]),trim($_POST["emergency_number"])]);
    ...
    // Self check-in for an event
    if ($action === "self_checkin") {
        $eventId = (int)$_POST["event_id"];
        $regId   = (int)$_POST["reg_id"];

        // Verify this registration belongs to the user and is approved
        $regCheck = $pdo->prepare("SELECT r.id FROM volunteer_registrations r WHERE r.id=? AND r.user_id=? AND r.status='approved'");
        $regCheck->execute([$regId, $userId]);
        if (!$regCheck->fetch()) {
            $error = "You cannot check in for this event.";
        } else {
            // Get event details
            $evt = $pdo->prepare("SELECT * FROM volunteer_events WHERE id=?");
            $evt->execute([$eventId]);
            $event = $evt->fetch();
            if (!$event) { $error = "Event not found."; }
            else {
                // Check not already checked in
                $dup = $pdo->prepare("SELECT id FROM volunteer_attendance WHERE registration_id=? AND event_name=? AND event_date=?");
                $dup->execute([$regId, $event['title'], $event['event_date']]);
                if ($dup->fetch()) {
                    $error = "You have already checked in for this event.";
                } else {
                    $pdo->prepare("INSERT INTO volunteer_attendance (registration_id,event_name,event_date,hours,status,recorded_by,submitted_by,confirmed) VALUES (?,?,?,?,'present',?,?,0)")
                        ->execute([$regId, $event['title'], $event['event_date'], $event['hours'], $userId, 'self']);
                    $success = "Check-in recorded! Pending admin confirmation.";
                    $tab = "attendance";
                }
            }
        }
...
$programs = $pdo->query("SELECT * FROM volunteer_programs WHERE is_active=1 ORDER BY type")->fetchAll();
...
// Events available for check-in (approved registrations only)
$availableEvents = $pdo->prepare(
    "SELECT ve.*, p.name as prog_name, r.id as reg_id
     FROM volunteer_events ve
     JOIN volunteer_programs p ON p.id=ve.program_id
     JOIN volunteer_registrations r ON r.program_id=ve.program_id AND r.user_id=? AND r.status='approved'
     WHERE ve.event_date <= CURDATE()
     AND NOT EXISTS (
       SELECT 1 FROM volunteer_attendance va
       WHERE va.registration_id=r.id AND va.event_name=ve.title AND va.event_date=ve.event_date
     )
     ORDER BY ve.event_date DESC"
);
$availableEvents->execute([$userId]);
$checkInEvents = $availableEvents->fetchAll();
...
<?php if (!empty($checkInEvents)): ?>
<div style="background:#e3f2fd;border:1.5px solid #90caf9;border-radius:12px;padding:16px 18px;margin-bottom:16px">
  <div style="font-size:.9rem;font-weight:700;color:#0d3b6e;margin-bottom:12px;display:flex;align-items:center;gap:7px">
    <i class="fas fa-qrcode"></i> Events Available for Check-in
  </div>
  <?php foreach ($checkInEvents as $evt): ?>
  <form method="POST" style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#fff;border-radius:9px;border:1px solid #e2e8f0;margin-bottom:8px;gap:12px;flex-wrap:wrap">
    <input type="hidden" name="action" value="self_checkin"/>
    <input type="hidden" name="event_id" value="<?=$evt['id']?>"/>
    <input type="hidden" name="reg_id" value="<?=$evt['reg_id']?>"/>
    <div>
      <div style="font-weight:700;font-size:.88rem"><?=htmlspecialchars($evt['title'])?></div>
      <div style="font-size:.75rem;color:#475569;margin-top:2px">
        <?=htmlspecialchars($evt['prog_name'])?> &middot;
        <?=date('M j, Y', strtotime($evt['event_date']))?> &middot;
        <?=$evt['hours']?> hrs
        <?php if ($evt['location']): ?>&middot; <?=htmlspecialchars($evt['location'])?><?php endif; ?>
      </div>
    </div>
    <button type="submit" style="padding:7px 16px;background:#1565c0;color:#fff;border:none;border-radius:8px;font-family:inherit;font-size:.82rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:6px;flex-shrink:0">
...
<div style="background:#fff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden">
  <table style="width:100%;border-collapse:collapse;font-size:.85rem">
    <thead><tr style="background:#f8fafc">
      <th style="padding:11px 16px;text-align:left;font-size:.75rem;font-weight:700;text-transform:uppercase;color:#475569;border-bottom:1px solid #e2e8f0">Event</th>
      <th style="padding:11px 16px;text-align:left;font-size:.75rem;font-weight:700;text-transform:uppercase;color:#475569;border-bottom:1px solid #e2e8f0">Program</th>
      <th style="padding:11px 16px;text-align:left;font-size:.75rem;font-weight:700;text-transform:uppercase;color:#475569;border-bottom:1px solid #e2e8f0">Date</th>
      <th style="padding:11px 16px;text-align:left;font-size:.75rem;font-weight:700;text-transform:uppercase;color:#475569;border-bottom:1px solid #e2e8f0">Hours</th>
      <th style="padding:11px 16px;text-align:left;font-size:.75rem;font-weight:700;text-transform:uppercase;color:#475569;border-bottom:1px solid #e2e8f0">Status</th>
      <th style="padding:11px 16px;text-align:left;font-size:.75rem;font-weight:700;text-transform:uppercase;color:#475569;border-bottom:1px solid #e2e8f0">Verified</th>
    </tr></thead>
    <tbody>
    ...
    <tr style="border-bottom:1px solid #f1f5f9">
      ...
      <td style="padding:10px 16px">
        <?php if ($a['confirmed']): ?>
          <span style="background:#e8f5e9;color:#2e7d32;padding:3px 9px;border-radius:50px;font-size:.72rem;font-weight:700"><i class="fas fa-check-circle"></i> Confirmed</span>
        <?php else: ?>
          <span style="background:#fff8e1;color:#f57f17;padding:3px 9px;border-radius:50px;font-size:.72rem;font-weight:700"><i class="fas fa-clock"></i> Pending</span>
        <?php endif; ?>
        <?php if (!empty($a['submitted_by']) && $a['submitted_by'] !== 'admin'): ?>
          <span style="font-size:.7rem;color:#94a3b8;display:block;margin-top:2px">via <?=ucfirst($a['submitted_by'])?></span>
        <?php endif; ?>
      </td>
    </tr>
    <?php endforeach; ?>
  ...
  <div class="modal-box">
    ...
    <div class="modal-body-inner">
      ...
      <form method="POST">
        ...
        <div class="fg"><label>School / Organization</label>
          <select name="school_org">
            <option value="">— Select or leave blank —</option>
            <?php
            $volOrgs = $pdo->query('SELECT name FROM organizations WHERE is_active=1 ORDER BY name')->fetchAll(PDO::FETCH_COLUMN);
            foreach ($volOrgs as $vo): ?>
            <option value="<?=htmlspecialchars($vo)?>" <?=($user['school_name']??'')===$vo?'selected':''?>><?=htmlspecialchars($vo)?></option>
            <?php endforeach; ?>
            <?php if ($user['school_name'] && !in_array($user['school_name'], $volOrgs)): ?>
            <option value="<?=htmlspecialchars($user['school_name'])?>" selected><?=htmlspecialchars($user['school_name'])?></option>
            <?php endif; ?>
          </select>
        </div>

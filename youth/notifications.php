...
$where  = $filter !== "all" ? "AND category=?" : "";
$params = $filter !== "all" ? [$userId, $filter] : [$userId];
$notifs = $pdo->prepare("SELECT * FROM notifications WHERE user_id=? $where ORDER BY created_at DESC LIMIT 100");
...
<div class="filter-tabs">
  <a href="?filter=all"         class="ftab <?=$filter==='all'?'active':''?>"><i class="fas fa-list"></i> All</a>
  <a href="?filter=event"       class="ftab <?=$filter==='event'?'active':''?>"><i class="fas fa-calendar-alt"></i> Events</a>
  <a href="?filter=scholarship" class="ftab <?=$filter==='scholarship'?'active':''?>"><i class="fas fa-graduation-cap"></i> Scholarship</a>
  <a href="?filter=approval"    class="ftab <?=$filter==='approval'?'active':''?>"><i class="fas fa-check-circle"></i> Approvals</a>
  <a href="?filter=assistance"  class="ftab <?=$filter==='assistance'?'active':''?>"><i class="fas fa-hands-helping"></i> Assistance</a>
  <a href="?filter=volunteer"   class="ftab <?=$filter==='volunteer'?'active':''?>"><i class="fas fa-user-check"></i> Volunteer</a>
</div>
...
<div style="background:#fff;border-radius:14px;border:1px solid #e2e8f0;box-shadow:0 2px 8px rgba(0,0,0,.06);overflow:hidden">
  ...
  <?php else: foreach ($notifications as $n):
    [$color,$bg,$icon] = $catColors[$n['category']] ?? ['#475569','#f1f5f9','fa-bell'];
  ?>
  <div class="notif-item <?=$n['is_read']?'':'notif-unread'?>">
    ...
    <div class="notif-content">
      ...
      <div class="notif-time">
        ...
        <span style="background:<?=$bg?>;color:<?=$color?>;padding:1px 8px;border-radius:50px;font-size:.68rem;font-weight:700;margin-left:4px"><?=ucfirst($n['category'])?></span>
      </div>
      <?php if ($n['link']): ?>

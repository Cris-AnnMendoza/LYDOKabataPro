...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['scanned_token'])){
    $token = trim($_POST['scanned_token']);
    
    if(!$token){
        $scanMessage = 'No QR code data received.';
    } else {
        // Try to find event by QR token (PostgreSQL compatible)
        $evStmt = $pdo->prepare('SELECT * FROM events WHERE qr_token = ? AND checkin_open = TRUE LIMIT 1');
        $evStmt->execute([$token]);
        $event = $evStmt->fetch();
        
        if(!$event){
            $scanMessage = 'Invalid QR code or event not found. Please try again.';
        } else {
...
<title>QR Scanner – LYDO Event Check-in</title>
...
<div class="scanner-container">
  ...
  <div class="scanner-box">
    <div class="scanner-header">
      ...
      <p>Scan the QR code displayed by the event organizer to check in</p>
    </div>

...
function generateWellbeingReply(string $input, array $user, PDO $pdo, int $userId, string $userType = 'youth'): string {
    ...
    return getAIResponse($input, $name, $userId, $userType);
}

// AI Response Generator
function getAIResponse(string $input, string $name, int $userId, string $userType): string {
    // Try Groq API
    $groqResponse = callGroqAPI($input, $name, $userType);
    if ($groqResponse) {
        return $groqResponse;
    }
...
function callGroqAPI(string $input, string $name, string $userType): ?string {
    ...
    $data = [
        'model' => 'openai/gpt-oss-120b',  // Your specific model
        'messages' => [
...
function getFallbackResponse(string $input, string $name): string {
    ...
    if (matchAny($text, ['hello','hi','hey','good morning','good afternoon','kumusta','kamusta'])) {
        return "Hello, **{$name}**! 😊 I'm your LYDO Well-being Assistant. I'm here to help you with anything you need - whether it's about mental health, school, relationships, career advice, technology, or just to chat about any topic you're curious about. What's on your mind today?";
    }
    
    if (matchAny($text, ['thank','thanks','salamat'])) {
        return "You're very welcome, **{$name}**! 💙 I'm always here whenever you need someone to talk to or if you have any questions about anything at all.";
    }
    
    if (matchAny($text, ['bye','goodbye','paalam','take care'])) {
        return "Take care, **{$name}**! 💙 Remember, I'm always here if you need to talk about anything. Have a wonderful day! 🌟";
    }
    
    // General intelligent response for any other topic
    return "That's an interesting topic, **{$name}**! 💙 I'd love to help you with that. Could you tell me a bit more about what specifically you'd like to know or discuss? \n\nI'm here to assist with:\n• Mental health & well-being\n• Academic concerns\n• Career guidance\n• Technology questions\n• Relationships\n• Personal development\n• Or really anything else you'd like to chat about!\n\nFeel free to share more details so I can give you the best possible advice. 😊";
}

        ...
        if($code===strtoupper(substr(md5($eid.$tok.$w.'CHECKOUT'),0,6))||$code===strtoupper(substr(md5($eid.$tok.($w-1).'CHECKOUT'),0,6))){
            ...
            if($et&&strlen($et)>0){
                ...
                if(date('H:i:s')<$et&&empty($c['checkout_open'])){
                    echo json_encode(['success'=>false,'message'=>'Check-out not yet available. Opens at '.date('g:i A',strtotime($et)).'.']);exit;
                }
            }
            $ev=$c; break;
        }
    }
    if(!$ev){echo json_encode(['success'=>false,'message'=>'Invalid checkout code. Use the red code shown on screen.']);exit;}
    $eid=(int)$ev['id'];
...
<div id="tab-checkin" class="tab-pane">
  ...
  <div id="panelIn" class="ev-card">
    <div class="ev-card-header" style="background:linear-gradient(135deg,#0d3b6e,#1565c0);color:#fff">
      <i class="fas fa-sign-in-alt"></i><h3 style="color:#fff">Check In to Event</h3>
    </div>
    <div class="ev-card-body">
      <div class="checkin-wrap">
        ...
        <div style="font-size:.85rem;color:var(--gray-600);text-align:center;max-width:380px;line-height:1.6">Use your camera to scan the QR code displayed by the event organizer. <strong style="color:#c62828">You only have 15 minutes from event start to check in!</strong></div>
        
        <a href="qr_scanner.php" class="btn-action btn-in btn-qr-scanner" style="text-decoration:none;background:linear-gradient(135deg,#1565c0,#2196f3);max-width:380px">
          <i class="fas fa-qrcode"></i>
          <span>Open QR Scanner</span>
          <i class="fas fa-arrow-right" style="margin-left:auto;opacity:.7"></i>
        </a>
        
        <div style="font-size:.78rem;color:var(--gray-400);text-align:center;max-width:380px;line-height:1.6">
          <i class="fas fa-info-circle"></i> Point your camera at the QR code displayed on the organizer's screen to automatically check in.
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="panelOut" class="ev-card" style="display:none">
    <div class="ev-card-header" style="background:linear-gradient(135deg,#1b5e20,#2e7d32);color:#fff">
      <i class="fas fa-sign-out-alt"></i><h3 style="color:#fff">Check Out of Event</h3>
    </div>
    <div class="ev-card-body">
      <div class="checkin-wrap">
        ...
        <div style="font-size:.85rem;color:var(--gray-600);text-align:center;max-width:380px;line-height:1.6">Use your camera to scan the <strong style="color:#c62828">red check-out QR code</strong> displayed by the event organizer after the event ends.</div>

        <a href="qr_scanner.php" class="btn-action btn-out btn-qr-scanner" style="text-decoration:none;background:linear-gradient(135deg,#1b5e20,#2e7d32);max-width:380px">
          <i class="fas fa-qrcode"></i>
          <span>Open QR Scanner</span>
          <i class="fas fa-arrow-right" style="margin-left:auto;opacity:.7"></i>
        </a>

        <div style="background:#e8f5e9;border-radius:10px;padding:12px 16px;font-size:.82rem;color:#2e7d32;max-width:380px;text-align:center;line-height:1.6"><i class="fas fa-star"></i> <strong>Merit points are added instantly!</strong> Just scan to check out and earn your points.</div>
      </div>
    </div>
  </div>
</div>
...
function doCheckin(){
  ...
  var photoFile=document.getElementById('photoIn').files[0];
  ...
  if(!photoFile){showR('In','error','fa-exclamation-circle','Selfie Required','Please upload a selfie photo as proof of attendance.');return;}
  ...
  var fd=new FormData();fd.append('ajax_scan','1');fd.append('qr_token',code);fd.append('checkin_photo',photoFile);
...
function doCheckout(){
  ...
  var photoFile=document.getElementById('photoOut').files[0];
  ...
  if(!photoFile){showR('Out','error','fa-exclamation-circle','Selfie Required','Please upload a selfie photo as proof of attendance.');return;}
  ...
  var fd=new FormData();fd.append('ajax_checkout','1');fd.append('qr_token',code);fd.append('checkout_photo',photoFile);
  fetch(window.location.href,{method:'POST',body:fd})
    .then(function(r){return r.text();})
    .then(function(t){
...
function openQRScanner() {
  ...
  html5QrCode.start(
    ...
    (decodedText) => {
      ...
      // If it's a full URL with token parameter
      if (decodedText.includes('token=')) {
        const urlParams = new URLSearchParams(decodedText.split('?')[1]);
        const token = urlParams.get('token');
        if (token) {
          // Extract rotating code from URL parameters if present
      ...
      // Auto-fill the code input and trigger check-in
      document.getElementById('codeIn').value = code.toUpperCase();
      showR('In', 'info', 'fa-qrcode', 'QR Code Scanned!', 'Code: ' + code + '. Click "Check In Now" to proceed.');
    },
    (errorMessage) => {
      // Scanning failed or no QR code detected
      console.log('QR scan error:', errorMessage);
    }
  ).catch((err) => {
    alert('Unable to start camera. Please grant camera permission and try again.');
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && document.getElementById('qrModal').style.display === 'block') {
    closeQRScanner();
  }
});
</script>

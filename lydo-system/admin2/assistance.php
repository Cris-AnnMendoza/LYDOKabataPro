...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_status' && $reqId) {
        ...
        $pdo->prepare('UPDATE assistance_requests SET status=?,scheduled_date=?,decline_reason=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')
        ...
        $sub->execute([$reqId]);
        $row = $sub->fetch();
        if ($row && $row['submitted_by']) notifyUser($pdo, $row['submitted_by'], 'assistance', "Your request #$reqId status updated to: " . ($statusLabels[$newStatus] ?? $newStatus), "assistance_view.php?id=$reqId");

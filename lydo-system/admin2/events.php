...
// Auto-generate checkin_code for events that don't have one
...
// ── Time-based rotating code logic ───────────────────────
// Code rotates every 30 seconds. Generated from: event_id + qr_token + current 30-sec window
// This means the code changes automatically — sharing it is useless after the window
function getRotatingCode(string $qrToken, int $eventId): string {
    $window = floor(time() / 30); // 30-second window
    return strtoupper(substr(md5($eventId . $qrToken . $window . 'CHECKIN'), 0, 6));
}

function getCheckoutCode(string $qrToken, int $eventId): string {
    $window = floor(time() / 30); // 30-second window — different salt
    return strtoupper(substr(md5($eventId . $qrToken . $window . 'CHECKOUT'), 0, 6));
}

function getPrevRotatingCode(string $qrToken, int $eventId): string {
    $window = floor(time() / 30) - 1;
    return strtoupper(substr(md5($eventId . $qrToken . $window . 'CHECKIN'), 0, 6));
}

function getPrevCheckoutCode(string $qrToken, int $eventId): string {
    $window = floor(time() / 30) - 1;
    return strtoupper(substr(md5($eventId . $qrToken . $window . 'CHECKOUT'), 0, 6));
}
      ...
      <?php else: foreach ($events as $i => $ev): ?>
        <tr>
          ...
          <td style="white-space:nowrap">
            <!-- Show QR -->
            <a href="event_qr.php?id=<?= $ev['id'] ?>" class="btn-icon teal" title="Show QR / Check-in Code"><i class="fas fa-qrcode"></i></a>
            <!-- Toggle check-in -->
            ...
            <form method="POST" style="display:inline">
              ...
              <input type="hidden" name="event_id" value="<?= $ev['id'] ?>"/>
              <button type="submit" class="btn-icon <?= $checkoutAvailable ? 'red' : '' ?>" style="<?= !$checkoutAvailable ? 'color:#f57f17;background:#fff8e1' : '' ?>" title="<?= $checkoutAvailable ? 'Close Check-out' : 'Open Check-out Now' ?>">
...
<?php $secsLeft = getSecondsUntilNextRotation(); ?>
var secsLeft = <?= $secsLeft ?>;

function updateCountdowns() {
  document.querySelectorAll('.countdown').forEach(function(el) {
    el.textContent = secsLeft + 's';
    // Turn red when < 10 seconds
    el.style.color = secsLeft < 10 ? '#c62828' : '#94a3b8';
  });
}

function updateCodes() {
  // Reload the page to get fresh rotating codes from server
  location.reload();
}
...
var countdownTimer = setInterval(function() {
  ...
  if (secsLeft <= 0) {
    ...
    if ((addModal && addModal.style.display === 'flex') || (editModal && editModal.style.display === 'flex')) {
      // Reset countdown without reloading
      secsLeft = 30;
      countdownTimer = setInterval(arguments.callee, 1000);
      updateCountdowns();
    } else {
      updateCodes();
    }
